...
if (AdminModel.count() === 0) {
  ...
  AdminModel.create({ username, password_hash: passwordHash });
...
kategoriData.forEach((row) => {
  if (!KategoriModel.findByNama(row.nama)) {
    KategoriModel.create(row);
  }
});

const getKategoriId = (nama) => KategoriModel.findByNama(nama).id;

// Koordinat contoh di wilayah Desa Kapur, Kecamatan Sungai Raya, Kabupaten Kubu Raya, Kalbar
// Titik acuan pusat desa: sekitar -0.073, 109.385 (dekat Jl. Desa Kapur Gg. Prona)
const umkmData = [
  {
    nama: 'Warung Makan Bu Siti',
    kategori_id: getKategoriId('Kuliner'),
  ...
  },
  {
    ...
    kategori_id: getKategoriId('Fashion'),
  ...
  },
  {
    ...
    kategori_id: getKategoriId('Kerajinan'),
  ...
  },
  {
    ...
    kategori_id: getKategoriId('Jasa'),
  ...
  },
  {
    ...
    kategori_id: getKategoriId('Lainnya'),
  ...
  {
    nama: 'Es Kopi Kapur',
    kategori_id: getKategoriId('Minuman'),
...
const existingCount = UmkmModel.findAll().length;

if (existingCount === 0) {
  umkmData.forEach((row) => UmkmModel.create(row));
